		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				if ($object->statut == 0 && $action == 'editline' && $user->hasRight('ficheinter', 'creer') && GETPOST('line_id', 'int') == $objp->rowid) {
					...
					if ($modelSelected->name === "Classique") {
						if (!empty($extrafields)) {
					...
					print "<h4>Corps de l'intervention</h4>";
					$doleditor = new DolEditor('np_desc', $modelSelected->content, '', 164, 'dolibarr_details', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_DETAILS'), ROWS_2, '90%');
					$doleditor->Create();
					print '</td>';
					...
					print '</tr>' . "\n";
					/*print '</table>';
					print '</tr>' . "\n";*/ //uncomment this also for model in update mode
			...
			if ($object->statut == 0 && $user->hasRight('ficheinter', 'creer') && $action <> 'editline' && empty($conf->global->FICHINTER_DISABLE_DETAILS)) {
				...
				print formForModels($modelsForFrom);
				print "</tr>";
				$modelSelected = ModelLineInter::getById(1, $db); // it is the default model ( 'Classique' , cf table)
				//choose model from dropdown
				if (GETPOSTISSET('modeline')) {
					$modelSelected = ModelLineInter::getById(GETPOST('modeline'), $db);
				}
				print '<tr class="oddeven nohover">' . "\n";
				...
				if (empty($conf->global->FICHINTER_EMPTY_LINE_DESC)) {
					...
					print "<h4>Corps de l'intervention:</h4>";
					require_once DOL_DOCUMENT_ROOT . '/core/class/doleditor.class.php';
					$doleditor = new DolEditor('np_desc', $modelSelected->content, '', 100, 'dolibarr_details', '', true, true, true, ROWS_2, '90%');
					$doleditor->Create();
					print '</td>';
					print "</tr>";
				}
...
function formForModels($models)
{
	...
	$out .= '<h5>' . $langs->trans('Choose a model') . ':</h5>';

	// Get the currently selected model ID or use the default (1)
	$id_selected = GETPOSTISSET('modeline') ? GETPOST('modeline') : 1;

	// Generate the dropdown list of available models
	$out .= $form->selectarray('modeline', $models, $id_selected, 1, 0, 0, "", 0, 0, 0, '', 'minwidth100', 1, '', 0, 1);

	// Generate the "Apply" button
	$out .= '<input type="submit" value="' . $langs->trans('Apply') . '" name="lineapply" id="lineapply" class="button buttongen reposition nomargintop nomarginbottom" >';

	// Add the refocus JavaScript to the form
	$out .= getRefocusJs();
